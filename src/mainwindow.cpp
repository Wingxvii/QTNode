...
#include "analyzer/sensorwindow.h"
...
{
    LOG_CAMPBELL() << "Job dock triggered";
    if (checked) {
        m_jobsDock->show();
        m_jobsDock->raise();
    }
...
{
    LOG_CAMPBELL() << "Properties dock has been triggered";
    if (checked) {
...
void MainWindow::onSensorDockTriggered(bool checked)
{
    if (checked)
    {
        m_sensorWindow->show();
...
{
    LOG_CAMPBELL() << "Undo slot called";
    m_undoStack->undo();
}

void MainWindow::on_actionRedo_triggered()
{
    LOG_CAMPBELL() << "redo slot called";
    m_undoStack->redo();
}
