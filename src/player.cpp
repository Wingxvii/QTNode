...
//Campbell Hamilton
#include "analyzer/scrubeventmanager.h"
...
Player::Player(ScrubEventManager* events, QWidget *parent)
    : QWidget(parent)
    , m_position(0)
    ...
    , m_currentTransport(0)
{
    ...
    m_tabs->addTab(tr("cats are the best"));
    m_tabs->addTab(tr("you are absolutely right"));
    ...
    //add grid layout for scurbar and eventbar
    scrubEventLayout = events;
    vlayout->addLayout(scrubEventLayout);

    // Add the scrub bar.
    m_scrubber = new ScrubBar(this);
    m_scrubber->setMargin(margins);
    ...
    scrubEventLayout->setMargin(margins);
    scrubEventLayout->setSelectionSize(14);
    scrubEventLayout->setOriginCorner(Qt::BottomLeftCorner);
    scrubEventLayout->addWidget(m_scrubber, 0, 1);

    connect(m_scrubber, SIGNAL(currentTime(int)), scrubEventLayout, SLOT(setTime(int)));
    connect(this, SIGNAL(videoTime(double)), scrubEventLayout, SLOT(setVideoDuration(double)));
...
{
    scrubEventLayout->setFPS(MLT.profile().fps());
    ...
    if (m_isSeekable) {
        ...
        double time = MLT.producer()->get_length();
        double fps =  MLT.producer()->get_fps();
        emit videoTime(time/fps);
...
void Player::setSensorTime(double time)
{
    scrubEventLayout->setSensorTime(time);
}

//end campbell functions
