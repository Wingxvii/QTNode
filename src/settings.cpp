...
//campbells code
void ShotcutSettings::setBlackList(std::vector<QString> blackList)
{
    settings.beginWriteArray("blacklist");
    for (int i = 0; i < blackList.size(); i++)
    {
        settings.setArrayIndex(i);
        settings.setValue("name", blackList.at(i));
    }
    settings.endArray();
}

std::vector<QString> ShotcutSettings::getBlackList()
{
    std::vector<QString> blackList;
    int size = settings.beginReadArray("blacklist");
    for (int i = 0; i < size; i++)
    {
        settings.setArrayIndex(i);
        blackList.push_back(settings.value("name").toString());
    }
    settings.endArray();
    return blackList;
}

void ShotcutSettings::clearBlackList()
{
    settings.remove("blacklist");
}


void ShotcutSettings::clearFilterList()
{
    settings.remove("clearlist");
}

void ShotcutSettings::setFilterList(std::vector<QString> filterlist)
{
    settings.beginWriteArray("filterlist");
    for (int i = 0; i < filterlist.size(); i++)
    {
        settings.setArrayIndex(i);
        settings.setValue("name", filterlist.at(i));
    }
    settings.endArray();
}

std::vector<QString> ShotcutSettings::getFilterList()
{
    std::vector<QString> filterlist;
    int size = settings.beginReadArray("filterlist");
    for (int i = 0; i < size; i++)
    {
        settings.setArrayIndex(i);
        filterlist.push_back(settings.value("name").toString());
    }
    settings.endArray();
    return filterlist;
}
